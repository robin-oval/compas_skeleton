...
def automated_smoothing_surface_constraints_gh(mesh, surface):
    """Apply automatically surface-related constraints to the vertices of a mesh to smooth: kinks, boundaries and surface.
    ...
    points = [RhinoPoint.from_guid(rs.AddPoint(point)) for point in surface_kinks(surface)]
    curves = [RhinoCurve.from_guid(guid) for guid in surface_borders(surface, border_type=0)]
...
def surface_borders(surface, border_type=0):
    curves = rs.DuplicateSurfaceBorder(surface, type=border_type)
...
def surface_kinks(surface, threshold=1e-3):
    kinks = []
    borders = surface_borders(surface, border_type=0)
