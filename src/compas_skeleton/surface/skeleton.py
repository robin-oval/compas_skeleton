...
class Skeleton(Mesh):
    ...
    def singular_points(self):
        ...
        """
        return [
            trimesh_face_circle(self, fkey)[0][0] for fkey in self.singular_faces()
        ]
    ...
    def lines(self):
        ...
        """
        return [
            (trimesh_face_circle(self, fkey)[0][0], trimesh_face_circle(self, nbr)[0][0])
            for fkey in self.faces()
            for nbr in self.face_neighbors(fkey)
            if fkey < nbr
            and geometric_key(trimesh_face_circle(self, fkey)[0][0])
            != geometric_key(trimesh_face_circle(self, nbr)[0][0])
        ]
