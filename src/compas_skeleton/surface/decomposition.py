...
from compas_skeleton.surface.skeleton import Skeleton
from compas_skeleton.utilities import list_split
...
class SkeletonDecomposition(Skeleton):
    ...
    def branches_singularity_to_singularity(self):
        ...
        """
        map_corners = [geometric_key(trimesh_face_circle(self, corner)[0][0]) for corner in self.corner_faces()]
        return [
    ...
    def branches_singularity_to_boundary(self):
        ...
        """
        return [
            [trimesh_face_circle(self, fkey)[0][0], self.vertex_coordinates(vkey)]
    ...
    def branches_boundary(self):
        ...
        """
        boundaries = [bdry for bdry in self.vertices_on_boundaries()]
    ...
    def branches_splitting_collapsed_boundaries(self):
        ...
        for polyedge in [bdry for bdry in self.vertices_on_boundaries()]:
            ...
            for vkey in new_splits:
                ...
                for edge in self.face_halfedges(fkey):
                    if vkey in edge and not self.is_edge_on_boundary(*edge):
                        new_branches += [[trimesh_face_circle(self, fkey)[0][0], self.vertex_coordinates(vkey_2)] for vkey_2 in edge]
    ...
    def branches_splitting_flipped_faces(self):
        ...
        """
        new_branches = []
        centre_to_fkey = {geometric_key(trimesh_face_circle(self, fkey)[0][0]): fkey for fkey in self.faces()}
                ...
                for point in seams:
                    ...
                    for edge in self.face_halfedges(fkey):
                        if not self.is_edge_on_boundary(*edge):
                            new_branches += [[trimesh_face_circle(self, fkey)[0][0], self.vertex_coordinates(vkey)] for vkey in edge]
                            break
    ...
    def quadrangulate_polygonal_faces(self):
        ...
        for mesh in meshes:
            ...
            for bdry in mesh.vertices_on_boundaries():
                for vkey in bdry[:-1]:
